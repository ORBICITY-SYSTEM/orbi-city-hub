...
export async function createBookingReview(review: Omit<BookingReview, 'id' | 'created_at' | 'updated_at'>): Promise<string> {
  ...
  const [result] = await db.execute(
    ...
    [
      review.user_id,
      review.booking_reservation_id || null,
      review.guest_name,
      review.guest_country || null,
      review.rating,
      review.comment || null,
      review.positive_comment || null,
      review.negative_comment || null,
...
export async function createOrUpdateMetrics(metrics: Omit<BookingMetrics, 'id' | 'created_at' | 'updated_at'>): Promise<void> {
  ...
  await db.execute(
    ...
    [
      metrics.user_id,
      metrics.metric_date,
      metrics.occupancy_rate || null,
      metrics.total_bookings,
      metrics.total_revenue,
      metrics.average_daily_rate || null,
      metrics.revenue_per_available_room || null,
      metrics.page_views,
      metrics.conversion_rate || null,
      metrics.search_ranking || null,
      metrics.review_score || null,
      metrics.total_reviews,
      metrics.new_reviews_today,
      metrics.area_avg_occupancy || null,
      metrics.area_avg_review_score || null,
      metrics.page_views_vs_competitors || null
    ]
  );
}
...
export async function getLatestMetrics(userId: number): Promise<BookingMetrics | null> {
  ...
  return (rows as BookingMetrics[])[0] || null;
}
...
export async function getButlerStats(userId: number, days: number = 30): Promise<{
  ...
  const revenueImpact = (revenueRows as any[])[0]?.revenue || 0;

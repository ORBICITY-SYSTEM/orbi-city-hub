...
const taskFormSchema = z.object({
  ...
  category: z.enum(["housekeeping", "inventory", "maintenance", "scheduling", "general"]),
...
const AI_AGENTS = [
  { id: "housekeeping", name: "Housekeeping Agent", icon: Package, color: "from-blue-500 to-cyan-500", category: "housekeeping" },
  { id: "inventory", name: "Inventory Agent", icon: Package, color: "from-green-500 to-emerald-500", category: "inventory" },
  { id: "maintenance", name: "Maintenance Agent", icon: Wrench, color: "from-orange-500 to-amber-500", category: "maintenance" },
  { id: "scheduling", name: "Scheduling Agent", icon: Calendar, color: "from-purple-500 to-violet-500", category: "scheduling" },
  { id: "automation", name: "Automation Agent", icon: Zap, color: "from-indigo-500 to-blue-500", category: "general" },
];

export default function AILogisticsDirector() {
  const { t } = useLanguage();
  ...
  const { data: tasksData, isLoading: isLoadingTasks } = trpc.logisticsAIDirector.getTasks.useQuery(
    {
      limit: pageSize,
  ...
  const { data: taskStats } = trpc.logisticsAIDirector.getTaskStats.useQuery(undefined, {
    staleTime: 60000,
    cacheTime: 300000,
  });

  const createTaskMutation = trpc.logisticsAIDirector.createTask.useMutation({
    onSuccess: () => {
      toast.success("ამოცანა შექმნილია");
      reset();
      utils.logisticsAIDirector.getTasks.invalidate();
      utils.logisticsAIDirector.getTaskStats.invalidate();
    },
    onError: (error) => {
  ...
  const updateTaskMutation = trpc.logisticsAIDirector.updateTaskStatus.useMutation({
    onSuccess: () => {
  ...
  const streamAIResponse = useCallback(async (action: string, message: string) => {
    ...
    try {
      const actionPrefixes: Record<string, string> = {
        daily_briefing: "დღის ბრიფინგი ლოჯისტიკისთვის: ",
        analyze_performance: "გააანალიზე ლოჯისტიკის შედეგები: ",
        create_plan: "ლოჯისტიკის გეგმის შექმნა: ",
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <Button
                  ...
                  onClick={() => handleQuickAction("create_plan", "ლოჯისტიკის გეგმის შექმნა")}
                >
          ...
          <Card className="min-h-[500px] flex flex-col">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Brain className="h-5 w-5 text-primary" />
                AI Logistics Director Chat
              </CardTitle>
              <CardDescription>
                დაწერე შეტყობინება AI Logistics Director-თან სასაუბროდ
              </CardDescription>
            </CardHeader>
